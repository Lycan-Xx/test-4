...
const PaymentHistoryList = () => {
 ...
 return (
  <section className="flex flex-col mx-auto w-full font-lato">
   ...
   <div className="flex flex-col lg:flex-row justify-between gap-4">
    <div className="lg:h-[41px] relative">
     <input
      className="lg:w-[449px] pl-[36px] w-full h-full rounded-[8px] bg-[#fbfbfb] focus:outline-none "
      type="text"
    ...
    <div className=" lg:flex lg:flex-row grid grid-cols-2 gap-[13px]">
     {Sorters.map((item) => {
      ...
      return (
       <button
        className="flex flex-row lg:justify-center items-center align-middle border-[0.5px] border-[#f0f2f4] rounded-lg h-[100%] p-2 gap-2 "
        key={id}
       >
    ...
    <table className="flex flex-col lg:mt-[23px] bg-[#eceff2] w-max lg:w-full">
     <thead className="bg-[#e4e6e8]">
      <tr className="flex flex-row justify-between items-center text-[14px] w-full h-[46px] lg:h-[52px] font-poppins  text-[#374151]">
       <th className=""></th>
       <th className="flex-grow-1 font-medium">Payment ID</th>
       <th className="font-medium">Status</th>
       <th className="font-medium">Amount</th>
       <th className="font-medium">Payment Method</th>
       <th className="font-medium">Creation date</th>
       <th></th>
      </tr>
     </thead>
     <tbody
      ...
      {currentItems.map((item) => {
       ...
       return (
        ...
        >
         ...
         <td className="lg:font-bold font-light text-sm leading-[140%] text-gray-700">
          {paymentID}
         </td>
         <td>
          {paymentStatus ? (
           <span className="bg-[#e7f6ec] rounded-3xl px-[7.5px] py-[5.5px] flex flex-row md:w-[105px] text-xs text-[#0f973d] font-lato font-bold leading-[16px]">
            {" "}
         ...
         <td className="text-sm font-bold text-gray-400 font-lato leading-[140%]">
          <span className="font-extrabold text-gray-700 pr-1">${Amount}</span>
          USD
         </td>
